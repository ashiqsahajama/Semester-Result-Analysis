...
public class studentcatana extends HttpServlet
{
	...
	public void doGet(HttpServletRequest rq, HttpServletResponse rp) throws ServletException, IOException
	{
        ...
        try
		{
            ...
            sql="select name from cat where roll_no='"+map.get("roll")+"'limit 1;";
            String name=null;
            rs=stmt.executeQuery(sql);
            if (rs.next()){
                name=rs.getString(1);
            ...
            out.println("<h2>Semester: "+map.get("sem")+ "</h2>");
            out.println("<h2>Subject: "+subject+"</h2>");
            
            String subincat="sub_"+i;
            int j=1;
            for (j=1;j<4;j++){
            sql="select "+subincat+" from cat where s_id='"+s_id+"' and Roll_No='"+ map.get("roll") +"' and cat_num='"+j+"';";
            //System.out.println(sql);
            String mark=null;
            rs=stmt.executeQuery(sql);
            if (rs.next()){
            ...
            sql="select "+subincat+" from cat where s_id='"+s_id+"' and cat_num='"+j+"';";
            //System.out.println(sql);
            rs=stmt.executeQuery(sql);
            int sum=0;
            int pass=0;
