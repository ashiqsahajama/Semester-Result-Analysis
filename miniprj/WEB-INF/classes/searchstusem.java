...
public class searchstusem extends HttpServlet
{
	...
	public void doPost(HttpServletRequest rq, HttpServletResponse rp) throws ServletException, IOException
	{
		...
		try
		{
			...
			String sql = "select se.s_id, name, roll_no, sub_1, sub_2, sub_3, sub_4, sub_5, sub_6 from semester as se join semm as s join batchh as b where b.id = s.id and s.s_id = se.s_id and s.sem ='"+ sem +"' and b.batch = '"+ bno +"' and b.dept = '"+ dep +"' and b.sec = '"+ sec +"' and se.roll_no = '"+ roll + "';";
...
"table,td,th"+"{"+
 ...
 "}" +"</style>";
            ...
            while(rs.next()) {
                table += "<th onclick=studsem('" + rs.getString(2) + "','" + roll + "','" + bno + "','" + sem + "','" + sec + "')>" + rs.getString(3) + "</th>" ;
            }
